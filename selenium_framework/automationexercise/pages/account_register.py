...
class AccountRegistrationPage:
    ...
    def fill_account_info(self):
        ...
        title_radio = s.driver.find_element(By.XPATH, self.indexed_title_radio(random.randint(1, 2)))
        title_radio.click()
        ...
        account_month = s.driver.find_element(By.XPATH, self.month_of_birth)
        selector = Select(account_month)
        selector.select_by_visible_text(random_date.split()[1])

        account_year = s.driver.find_element(By.XPATH, self.years_of_birth)
        selector = Select(account_year)
        selector.select_by_value(random_date.split()[2])

        account_first_name = s.driver.find_element(By.XPATH, self.account_info_first_name)
        account_first_name.send_keys(fake.first_name())
        account_last_name = s.driver.find_element(By.XPATH, self.account_info_last_name)
        account_last_name.send_keys(fake.last_name())
        account_company = s.driver.find_element(By.XPATH, self.account_info_company)
        account_company.send_keys(fake.company())

        countries = s.driver.find_elements(By.XPATH, self.countries_name)
        countries_name = [country.text for country in countries]
        account_country = s.driver.find_element(By.XPATH, self.country)
        selector = Select(account_country)
        selector.select_by_visible_text(random.choice(countries_name))

        account_address = s.driver.find_element(By.XPATH, self.account_info_address)
        account_address.send_keys(fake.address())
        account_state = s.driver.find_element(By.XPATH, self.account_info_state)
        account_state.send_keys(fake.state())
        account_city = s.driver.find_element(By.XPATH, self.account_info_city)
        account_city.send_keys(fake.city())
        account_zipcode = s.driver.find_element(By.XPATH, self.account_info_zipcode)
        account_zipcode.send_keys(fake.postcode())
        account_mobile_number = s.driver.find_element(By.XPATH, self.account_info_mobile_number)
        account_mobile_number.send_keys(fake.phone_number())

        account_checkboxes = s.driver.find_elements(By.XPATH, self.account_info_checkboxes)
        for checkbox in account_checkboxes:
            checkbox.click()

        create_account_btn = s.driver.find_element(By.XPATH, self.create_account_btn)
    ...
    def delete_account(self):
        _delete_account_btn = s.driver.find_element(By.XPATH, self.delete_account_btn)
